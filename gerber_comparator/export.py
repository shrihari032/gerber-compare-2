...
def export_package(result: ComparisonResult, output_dir: str | Path) -> Path:
    ...
    (data / "raw_differences.json").write_text(json.dumps({"xor": _geojson(result.raw_xor), "missing_from_working": _geojson(result.missing_geometry), "added_in_working": _geojson(result.added_geometry)}, indent=2), encoding="utf-8")
