...
def normalize_geometry(geometry, *, label: str = "Geometry", warnings: list[str] | None = None):
    ...
    polygons = _polygonal_components(repaired)
    if not polygons:
        raise GeometryError(f"{label} is invalid and repair produced no polygonal copper geometry.")
    try:
        normalized = unary_union(polygons)
    except Exception as exc:
        raise GeometryError(f"{label} is invalid and polygonal repair could not be unioned safely.") from exc
...
def safe_union(geometries: Iterable, *, label: str = "Geometry", warnings: list[str] | None = None):
    ...
    try:
        return normalize_geometry(unary_union(inputs), label=label, warnings=warnings)
    except Exception as first_error:
        repaired = [normalize_geometry(item, label=f"{label} primitive {index}", warnings=warnings) for index, item in enumerate(inputs, 1)]
        try:
...
def safe_symmetric_difference(original, working, *, warnings: list[str] | None = None):
    ...
    try:
        ...
        try:
            ...
            raise GeometryError("Raw XOR failed after geometry repair; this Shapely version has no precision retry.") from exc
        # 1 nm is far below PCB comparison tolerances and is used only after GEOS fails.
        precision_original = set_precision(original, 0.000001)
        precision_working = set_precision(working, 0.000001)
        try:
