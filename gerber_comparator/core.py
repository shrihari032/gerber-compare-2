...
import time
from .geometry import normalize_geometry, resolve_geometry, safe_difference, safe_symmetric_difference, safe_union
...
class ComparisonResult:
    ...
    def statistics(self):
        reasons = [region.classification_reason for region in self.regions]
        return {"total_difference_regions": len(self.regions), "flagged_regions": sum(r.classification == "FLAG" for r in self.regions), "ignored_regions": sum(r.classification == "IGNORE" for r in self.regions), "unresolved_regions": sum(r.classification == "UNRESOLVED" for r in self.regions), "missing_from_working": reasons.count("MISSING_FROM_WORKING"), "added_in_working": reasons.count("ADDED_IN_WORKING"), "geometry_changes": reasons.count("GEOMETRY_CHANGE"), "translation_changes": reasons.count("TRANSLATION"), "topology_changes": reasons.count("TOPOLOGY_CHANGE"), "total_xor_area_mm2": self.raw_xor.area, "flagged_xor_area_mm2": self.flagged_xor.area}
...
def _local_deviation(region, original, working, margin):
    ...
    local_original = normalize_geometry(local_original, label="Local deviation original")
    local_working = normalize_geometry(local_working, label="Local deviation working")
    if local_original.is_empty or local_working.is_empty: return float("inf")
    ...
    if local_original.is_valid and local_working.is_valid:
...
def compare_gerbers(original: str | Path, working: str | Path, config: ComparisonConfig | None = None) -> ComparisonResult:
    ...
    directional_parts = [(part, "MISSING_FROM_WORKING") for part in _parts(missing_geometry)]
    directional_parts += [(part, "ADDED_IN_WORKING") for part in _parts(added_geometry)]
    # Directional differences partition raw XOR. The raw-XOR fallback protects
    # diagnostics if a GEOS overlay returns an unexpected empty direction.
    if not directional_parts and not raw_xor.is_empty:
        directional_parts = [(part, "GEOMETRY_CHANGE") for part in _parts(raw_xor)]
    for index, (part, direction) in enumerate(directional_parts, 1):
        ...
        if topology_changed and config.topology_check and not original_geometry.is_empty and not aligned_working.is_empty:
            classification, reason = "FLAG", "TOPOLOGY_CHANGE"
        elif direction in {"MISSING_FROM_WORKING", "ADDED_IN_WORKING"} and deviation > config.geometric_tolerance_mm:
            classification, reason = "FLAG", direction
    ...
    result = ComparisonResult(original_layer, working_layer, config, original_geometry, aligned_working, raw_xor, flagged_xor, regions, {"translation_x_mm": dx, "translation_y_mm": dy, "rotation_deg": rotation, "method": "centroid" if config.auto_alignment else "manual"}, missing_geometry, added_geometry, repair_warnings, {"parse_seconds": parsed-started, "geometry_seconds": geometry_time-parsed, "xor_and_analysis_seconds": time.perf_counter()-xor_time, "total_seconds": time.perf_counter()-started}, diagnostics)
    return result
